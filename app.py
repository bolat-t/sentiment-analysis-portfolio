...
from google_sheets_logger import initialise_logger, load_google_credentials
...
def load_custom_css():
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    h1 { background: linear-gradient(135deg, #a78bfa 0%, #14b8a6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .stTextArea textarea { background-color: #1e293b !important; border: 1px solid #475569 !important; border-radius: 0.5rem !important; color: #e5e7eb !important; }
    .stButton button[kind="primary"] { background: linear-gradient(135deg, #14b8a6 0%, #0d9488 100%) !important; color: #ffffff !important; border-radius: 0.5rem !important; padding: 0.75rem 1.5rem !important; font-weight: 600 !important; box-shadow: 0 4px 14px rgba(20, 184, 166, 0.4) !important; }
    .stButton button[kind="secondary"] { background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%) !important; color: #ffffff !important; }
    [data-testid="stMetric"] { background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #334155; border-radius: 0.75rem; padding: 1.5rem; }
    [data-testid="stMetric"] [data-testid="stMetricValue"] { color: #14b8a6 !important; font-size: 2rem !important; font-weight: 700 !important; }
    .hero-section { background: radial-gradient(circle at 20% 30%, rgba(124, 58, 237, 0.35), transparent 40%), radial-gradient(circle at 80% 25%, rgba(236, 72, 153, 0.35), transparent 40%); padding: 3rem; border-radius: 1rem; margin-bottom: 2rem; border: 1px solid #334155; }
    .model-card { background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #334155; border-radius: 0.75rem; padding: 1.5rem; transition: all 0.3s ease; }
    .model-card:hover { transform: translateY(-4px); box-shadow: 0 18px 40px rgba(0, 0, 0, 0.6); border-color: #14b8a6; }
    .positive-card { border-left: 3px solid #34d399 !important; }
    .negative-card { border-left: 3px solid #f87171 !important; }
    .tag-pill { background-color: rgba(20, 184, 166, 0.2); color: #5eead4; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.875rem; margin: 0.25rem; border: 1px solid rgba(20, 184, 166, 0.3); }
    .feature-icon { width: 8px; height: 8px; background: linear-gradient(135deg, #14b8a6 0%, #a78bfa 100%); border-radius: 50%; }
...
# Initialize Google Sheets logger once
if 'logger' not in st.session_state:
    ...
    try:
        ...
        if credentials:
            ...
            st.warning("⚠️ Google Sheets logging not configured, using local JSON logs only")
